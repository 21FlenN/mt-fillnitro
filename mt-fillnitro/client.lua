        ...
        options = {
            {
                ...
                label = 'Fill nitro', -- This is the label of this option which you would be able to click on to trigger everything, this has to be a string
...
RegisterNetEvent('mt-fillnitro:checaritem', function()
    QBCore.Functions.TriggerCallback('mt-fillnitro:server:tensoitemdonitro', function(HasItems)
        if HasItems then
            local success = exports['qb-phone']:PhoneNotification('Fill the nitro', 'Fill the nitro for 2 gne?', "fas fa-fill", "#25D366", '10000', 'fas fa-check-circle', 'fas fa-times-circle')
            if success then
                exports["rpemotes"]:EmoteCommandStart('mechanic4')
                QBCore.Functions.Progressbar('teste', 'Filling up the nitro', 2500, false, true, {
            ...
            TriggerEvent('QBCore:Notify', "You don't have a bottle of nitro or a phone", 'error')
        end
    end)
end)